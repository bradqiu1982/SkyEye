...
namespace SkyEye.Models
{
    public class CfgUtility
    {
        public static Dictionary<string, string> GetSysConfig(Controller ctrl)
        {
            ...
            if (ctrl != null)
            {
                lines = System.IO.File.ReadAllLines(ctrl.Server.MapPath("~/Scripts/SkyEyeCfg.txt")).ToList();
            }
            else
            {
                lines = System.IO.File.ReadAllLines(AppDomain.CurrentDomain.BaseDirectory+ @"\Scripts\SkyEyeCfg.txt").ToList();
            }
